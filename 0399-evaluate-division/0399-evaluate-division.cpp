...
public:
    ...
    unordered_map<string,unordered_map<string,double>>buildGraph(const vector<vector<string>>& equation,const vector<double>& values){
        unordered_map<string,unordered_map<string,double>>gr;
        for(int i=0;i<equation.size();i++){
            string dividend=equation[i][0];
            string divisor=equation[i][1];
